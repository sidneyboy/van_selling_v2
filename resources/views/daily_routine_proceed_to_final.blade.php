<form method="post" action="{{ route('daily_routine_submit_sales_order_and_proceed') }}" accept-charset="UTF-8">
@csrf
		...
		<table class="table table-bordered table-hover" id="daily_routine_sales_order_export">
			<thead>
				<tr>
					...
					<th>MOT</th>
					<th>
						<input type="hidden" name="sales_order_number" value="{{ 'SOCASE-'.$sales_order_name ."". $time }}">
						SO NUMBER
					</th>
					<th>{{ $principal->principal }}</th>
					<th style="text-transform: uppercase;">{{ $agent->name }}</th>
					<th>{{ $customer_location->location->location }}</th>
			...
			<tbody>
				@foreach($sku_data as $data)
					<tr>
						<td>SALES ORDER CASE</td>
						<td>{{ $mode_of_transaction }}</td>
						<td>{{ 'SOCASE-'.$sales_order_name ."". $time }}</td>
						<td>{{ $principal_id }}</td>
						<td>{{ auth()->user()->id }}</td>
						<td>{{ $customer_location->location->id }}</td>
						<td>{{ $customer_id }}</td>
						<td>{{ $data->id }}</td>
						<td>
	...
	<button class="btn btn-block btn-success" onclick="exportTableToCSV('{{ 'SOCASE-'.$sales_order_name ."-". $date }}.csv')">Export HTML Table To CSV File</button>
</form>
	...
	function exportTableToCSV(filename) {
	 ...
	 Swal.fire({
	   ...
	   timer: 1500
	    ...
	    var csv = [];
	    var rows = document.querySelectorAll("#daily_routine_sales_order_export tr");
