...
class Van_selling_ledger_controller extends Controller
{
    public function index()
    {
        ...
        return view('van_selling_ledger')->with('active','van_selling_ledger')
    ...
    public function van_selling_ledger_generate(Request $request)
    {
        ...
        $van_selling_details = DB::table('Van_selling_upload_ledgers')
                ->select('principal','sku_code','unit_of_measurement','description','beg','butal_equivalent', DB::raw('SUM(sales) as total_sales'),DB::raw('SUM(van_load) as total_van_load'),DB::raw('SUM(adjustments) as total_adjustments'))
